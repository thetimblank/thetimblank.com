...
interface P extends React.HTMLAttributes<HTMLDivElement> {
    ...
    autoplay?: number;
    contextItems?: Item[];
    setAutoplayEnabled?: React.Dispatch<
...
const Lightbox: React.FC<P> = ({
    ...
    const [currentIndex, setCurrentIndex] = useState(
        contextItems
            ? contextItems.findIndex((current) => item.image === current.image)
            : 0
    );
    ...
    const prevImage = () => {
        ...
        setCurrentIndex(
            (prevIndex) =>
                (prevIndex - 1 + contextItems.length) % contextItems.length
        );
    };
            ...
            {shown && (
                ...
                >
                    {contextItems && contextItems.length > 1 && (
                        <button
                            className='flex items-center justify-start rounded-r-4xl absolute left-0 pl-5 sm:pl-10 h-full w-1/4 z-10 text-light-500 hover:text-light-900 bg-gradient-to-r from-black/25 hover:from-accent/50 focus-visible:from-accent/50 transition-colors cursor-pointer outline-0'
                            onClick={prevImage}
                        >
                    ...
                    {contextItems && contextItems.length > 1 && (
                        <button
                            className='flex items-center justify-end rounded-l-4xl absolute right-0 pr-5 sm:pr-10 h-full w-1/4 z-10 text-light-500 hover:text-light-900 bg-gradient-to-l from-black/25 hover:from-accent/50 focus-visible:from-accent/50 transition-colors cursor-pointer outline-0'
                            onClick={nextImage}
                        >
