...
const Carousel: React.FC<P> = ({
    ...
    const extendedImages = [
        ...items.slice(items.length - clones),
        ...items,
    ...
    return (
        <div
            className={clsx('w-screen py-2 overflow-x-hidden', className)}
            {...props}
        >
            <div
                className='flex'
                style={{
                    ...
                    transform: `translateX(-${
                        index * (itemWidth + gap) - centerOffset
                    }px)`,
            ...
            >
                {extendedImages.map((item, idx) => (
                    <button
                        ...
                        onClick={() =>
                            openLightbox({ image: item.image }, items)
                        }
                    >
                        <Image
                            src={item.image}
            ...
            <div className='flex justify-between w-full gap-3 mt-3 px-[25%]'>
                {width > 640 ? (
                    <div className='flex flex-col gap-1 px-6 py-4 bg-light-900/40 dark:bg-dark-900/40 backdrop-blur-xl shadow-sm text-dark-500 dark:text-light-100 rounded-xl'>
                        {items[current].name && (
                            <p className='md:text-3xl font-title font-bold'>
                                {items[current].name}
                            </p>
                        )}
                        {items[current].url && (
